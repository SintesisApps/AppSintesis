...
$select_app="SELECT * FROM app_articulos WHERE posicion='Slide-Principal-Interior' and estatus='1' and plaza='nacionales' and DATE_SUB(CURDATE(),INTERVAL 7 DAY) <= fecha
							UNION
	...
	if($id_seccion==7)
	{
		$c_co="SELECT * FROM columnistas WHERE nombre_completo LIKE '%".$ap."%'";
	...
	$imagen_p=utf8_decode($imagen_p);
	
		
		$slide_principal.='
		<a href="#nota" onclick="LeerNota('.$id_nota_app.')">
              <div class="ContenidoPrincipalSeccion">
                <div class="ImagenPrincipalSeccion">';
				if($id_seccion==7)
				{
					$slide_principal.='<img src="'.$url_dominio_.'/images/'.$imagen_p.'" >';
					}
				else
				{$slide_principal.='<img src="'.$url_dominio_.'/images/imagenes-articulos/'.$imagenPlaza.'" >';}
			...
			if($Id_SeccionSlideVertical==7)
				{
					$c_co="SELECT * FROM columnistas WHERE nombre_completo LIKE '%".$AutorSlideVertical."%'";
									...
									while($f_co=mysql_fetch_assoc($r_co)):
										...
										$foto=utf8_encode($foto);
									endwhile;
									$imagen_art="imagenes-columnistas/".$foto;
				}
				$imagen_art=utf8_decode($imagen_art);
				
			
				$slide_vertical.=' <div > <a href="#nota" onclick="LeerNota('.$id_nota_app.')">
					  <div class="SlideVerticalArticulo">
					   
						<div class="SlideVerticalImagen">';
						if($Id_SeccionSlideVertical==7)
						{$slide_vertical.='<img src="'.$url_dominio_.'/images/'.$imagen_art.'" style="height:140px;"  >';}
						else
						{$slide_vertical.='<img src="'.$url_dominio_.'/images/imagenes-articulos/'.$imagen.'" >';}
						
						//<img src="'.$url_dominio_.'/images/imagenes-articulos/'.$imagen.'" >
						$slide_vertical.='</div>
