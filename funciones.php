...
//Función para Extraer una Video de un texto
function extraer_video($cadena){
	
	$maximo= strlen($cadena); 
	$ide= 'http://youtube.com/v/'; 
	$ide2= '?fs'; 
	$total= strpos($cadena,$ide); 
	$total2= stripos($cadena,$ide2); 
	$total3= ($maximo-$total2-0); 
	$video= substr ($cadena,$total+21,-$total3-0); 
...
function imprimir_publicidad($archivo,$codigo,$url_destino,$ancho,$alto){
	...
	$url_dominio_="http://sintesis.mx";
	$extension = end($exten);
	
	
	if($archivo):
			if($extension=="swf"):
			$res='<object id="FlashID" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="'.$ancho.'" height="'.$alto.'">
						  ...
						  <!--[if !IE]>-->
						  <object type="application/x-shockwave-flash" data="'.$url_dominio_.'/images/imagenes-publicidad/'.$archivo.'" width="'.$ancho.'" height="'.$alto.'" >
							<!--<![endif]-->
							<param name="quality" value="high" />
							<param name="wmode" value="opaque" />
							<param name="swfversion" value="6.0.65.0" />
...
//Crear thumbnial de PDF para Impresos Diarios

function CrearImagendeImpresoAPP($url_dominio_,$path,$RutaImagen){
$directorio=dir($path);
					...
					$pdf=$archivo;
					  ...
					  $archivo=str_replace(".pdf","",$archivo);
					   ...
					   if(!file_exists($RutaImagen.$archivo.".jpg")):
						...
						$html="<a href='".$url_dominio_."/".$path.$pdf."' target='new'><img src='".$url_dominio_."/".$RutaImagen.$archivo.".jpg' width='130' height='241'></a>";
...
function AbrirImagendeImpresoAPP($url_dominio_,$path,$RutaImagen){
$directorio=dir($path);
		...
		if (strtolower(substr($archivo, -3) == "pdf")):
			...
			if($archivofecha==date("dmY") && $ImagenPortada==01):
					...
					$archivo=str_replace(".pdf","",$archivo);
						...
						$html="<a href='".$url_dominio_."/".$path.$pdf."' target='new'><img src='".$url_dominio_."/".$RutaImagen.$archivo.".jpg' width='300'></a>";
...
function CrearImagendeSemanariosAPP($url_dominio_,$path,$RutaImagen){
					...
					$pdf=$archivo;
					  ...
					  $file_extension = array_pop($file_extension);
					   ...
					   if(!file_exists($RutaImagen.$archivo.".jpg")):
							  ...
							  $img->setImageFormat("jpg");
							  $img->thumbnailImage(300, 0);
							  $img->writeImages($RutaImagen.max($archivofecha).".jpg", true);
							   
						endif;
						
						$html="<a href='".$url_dominio_."/".$path.max($archivofecha).".pdf' target='new'><img src='".$url_dominio_."/".$RutaImagen.max($archivofecha).".jpg' width='130' height='241'></a>";
...
function AbrirImagendeSemanariosAPP($url_dominio_,$path,$RutaImagen){
		...
		if (strtolower(substr($archivo, -3) == "pdf")):
			...
			if($NombreImagen==01 && substr($archivo, 4,4)==date("Y") && substr($archivo, 2,2)==date("m")):
					...
					$file_extension = array_pop($file_extension);
						...
						$html="<a href='".$url_dominio_."/".$path.max($archivofecha).".pdf' target='new'><img src='".$url_dominio_."/".$RutaImagen.max($archivofecha).".jpg' width='300'></a>";
